...
namespace XOProject.Tests
{
    public class ShareControllerTests
    {
        ...
        public async Task Get_ShouldReturn_value(string symbol, bool expectedResult)
        {
            ...
            //Arrange
            List<HourlyShareRate> share =  new List<HourlyShareRate>()
            {
                new HourlyShareRate()
                {
                Symbol = "CBI",
            ...
            _shareRepositoryMock.Setup(x => x.Query()).Returns(share.AsQueryable());
            //Act 
            var result = await _shareController.Get(symbol) as ObjectResult;

            var objectReturned = result.Value as IQueryable<HourlyShareRate>;
            // Assert
